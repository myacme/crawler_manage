 ...
 */
@Component
public class ZgzyZsjgCgzxProcessor implements PageProcessor, Crawler {
	...
	private static final String SOURCE = "中共中央直属机关采购中心";
	...
	public static final String URL1 = "http://zzcg.ccgp.gov.cn/zbgg/index%s.jhtml";
	public static final String URL2 = "http://zzcg.ccgp.gov.cn/bggg/index%s.jhtml";
	public static final String URL3 = "http://zzcg.ccgp.gov.cn/jggg/index%s.jhtml";
	public static final String URL4 = "http://zzcg.ccgp.gov.cn/qtgg/index%s.jhtml";


	@Override
	public void process(Page page) {
		Document doc = Jsoup.parse(page.getRawText());
		Elements select = doc.select("div.List2.Top8 > ul > li");
		if (select != null && select.size() != 0) {
			...
			if (page.getRequest().getExtra("title") != null) {
				//爬取详情页
				String content = doc.select("div.Content.BorderTopDot > div").text();
	...
	public void run() {
		try {
			commonService.insertLogInfo(SOURCE, ZgzyZsjgCgzxProcessor.class.getName(), "begin", "");
			...
			for (int i = 0; i < 4; i++) {
				ZgzyZsjgCgzxProcessor bean = new ZgzyZsjgCgzxProcessor();
				bean.setUrl(i);
			...
			commonService.insertLogInfo(SOURCE, ZgzyZsjgCgzxProcessor.class.getName(), "success", "");
		} catch (Exception e) {
			e.printStackTrace();
			commonService.insertLogInfo(SOURCE, ZgzyZsjgCgzxProcessor.class.getName(), "error", e.getMessage());
		}
	}
}