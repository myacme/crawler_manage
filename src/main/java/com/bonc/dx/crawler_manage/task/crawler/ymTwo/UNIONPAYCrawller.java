 ...
 */
@Component
public class UNIONPAYCrawller implements Crawler {
	...
	private static Logger log = LoggerFactory.getLogger(UNIONPAYCrawller.class);
	private  long ct = 0;
	private  boolean isNext = true;
	//测试用表
	private static final String TABLE_NAME = "data_ccgp_henan_info";
	private static final String SOURCE = "中国银联";
	private static final String CITY = "金融网站";
	private  String begin_time;
	private  String end_time;
	public static  final SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd");
	private static String reg = "https://supplier.unionpay.com/jcepp-esp/notice/noticePageList?page=1";
	private static String fix = "https://supplier.unionpay.com";
...
//		WebDriver driver2 = null;
		try {
			commonService.insertLogInfo(SOURCE,UNIONPAYCrawller.class.getName(),"begin","");
...
//			driver2.manage().timeouts().implicitlyWait(1, TimeUnit.SECONDS);
			...
			for (int i = 0; i < 1; i++) {
				...
				while (isNext) {
					...
					Elements lis = list_doc.select("div.contentBox > div");
//				List<WebElement> lis = driver.findElements(By.cssSelector("div.text_con > div.text_row"));
					for (Element li : lis) {
						if (li.attr("class") != null && !li.attr("class").equals("")){
							continue;
						}
						String title = li.select("div.content > a").text();
						type = li.select("div.title").text().replace("[","").replace("]","");
						String date = li.select("div.date").text();
						String url = li.select("div.content > a").attr("href");
						url = fix+url;
						...
						if (date.equals("") || !date.contains("-") || simpleDateFormat.parse(end_time).before(simpleDateFormat.parse(date))) {
							...
							String content = doc.select("div.contentBox").text();
							if (content.equals("")){
								content = doc.text();
							}
							//加入实体类 入库
...
//							commonService.insertTable(insertMap, TABLE_NAME);
					...
					if (isNext) {
						...
						WebElement next = driver.findElement(By.cssSelector("div.page_bg > a:nth-child(7)"));
						if (next.getAttribute("class") != null && next.getAttribute("class").equals("disable")){
							break;
						}
						next.click();
						Thread.sleep(1000);
            ...
            commonService.insertLogInfo(SOURCE,UNIONPAYCrawller.class.getName(),"success","");
		} catch (Exception e) {
			e.printStackTrace();
			commonService.insertLogInfo(SOURCE,UNIONPAYCrawller.class.getName(),"error",e.getMessage());
		} finally {
			if(driver != null){
