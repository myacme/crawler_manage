...
public class HtgsCcgpCrawler implements Crawler {
	...
	public void run() {
		...
		int flag = 0;
		ChromeDriverPro driver = driverPool.get();
		recursionGet(origin, driver, days, flag);
		System.out.println("exit");
	}

	public void recursionGet(String origin, ChromeDriverPro driver, Map<String, String> days, int flag) {
		try {
			driver.get(origin);
			Thread.sleep(1000 * 1);
	 ...
	 * @return
	 */
	public String fateadm(String code, ChromeDriverPro driver, Map<String, String> days) {
		try {
	...
	public void getData(ChromeDriverPro driver, String origin) {
		driver.get(origin);
